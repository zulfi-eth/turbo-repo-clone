...
var getFlattenedValue = (node) => node.children.map((child) => "children" in child ? getFlattenedValue(child) : "value" in child ? child.value : "").join("");
...
var remarkStaticImage = ({ allowFutureImage, filePath }) => (tree, _file, done) => {
  const importsToInject = [];
  visit3(tree, "image", (node) => {
    let { url } = node;
    if (!url) {
      console.warn(`[nextra] File "${filePath}" contain image with empty "src" property, skipping\u2026`);
      return;
    }
    ...
    if (url.startsWith("/")) {
      ...
      if (!existsSync(urlPath)) {
        console.error(`[nextra] File "${filePath}" contain image with url "${url}" that not found in "/public" directory, skipping\u2026`);
        return;
      }
      url = urlPath;
    }
  ...
  tree.children.unshift(getASTNodeImport("$NextImageNextra", allowFutureImage ? "next/future/image" : "next/image"), ...importsToInject);
  done();
};
