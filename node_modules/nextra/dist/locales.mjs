...
function locales(request) {
  ...
  if (localeInPath) {
    ...
    if (finalLocale !== nextUrl.defaultLocale) {
      return NextResponse.redirect(new URL("/" + finalLocale + nextUrl.pathname + nextUrl.search, request.url));
    }
  }
  ...
  if (!pathname.endsWith("." + finalLocale)) {
    return NextResponse.rewrite(new URL("/" + finalLocale + pathname + "." + finalLocale + nextUrl.search, request.url));
  }
}
