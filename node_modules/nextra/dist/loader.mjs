  ...
  return __async(this, arguments, function* (dir, route = "/", fileMap = /* @__PURE__ */ Object.create(null)) {
    ...
    const promises = files.map((f) => __async(this, null, function* () {
      ...
      if (fileName === "meta.json") {
        console.warn('[nextra] "meta.json" was renamed to "_meta.json". Rename the following file:', path3.relative(CWD, filePath));
      }
    }));
    const items = (yield Promise.all(promises)).filter(truthy);
    const mdxPages = items.filter((item) => item.kind === "MdxPage");
    const locales = mdxPages.map((item) => item.locale);
    for (const locale of locales) {
      const metaIndex = items.findIndex((item) => item.kind === "Meta" && item.locale === locale);
      const defaultMeta = mdxPages.filter((item) => item.locale === locale).map((item) => {
        var _a2;
        return [
          item.name,
          ((_a2 = item.frontMatter) == null ? void 0 : _a2.title) || title(item.name.replace(/[-_]/g, " "))
        ];
      });
      const metaFilename = locale ? META_FILENAME.replace(".", `.${locale}.`) : META_FILENAME;
...
function loader(context, source) {
  return __async(this, null, function* () {
    ...
    const { result, headings, structurizedData, hasJsxInH1, readingTime } = yield compileMdx(content, {
      mdxOptions,
      unstable_readingTime,
      unstable_defaultShowCopyCode,
      unstable_staticImage,
...
export default MDXContent`.trimStart();
    ...
    const { route, title: title2, pageMap } = getPageMap({
      filePath: mdxPath,
      fileMap,
      defaultLocale,
      pageMap: items
    ...
    if (repository && gitRoot) {
      try {
        timestamp = yield repository.getFileLatestModifiedDateAsync(path4.relative(gitRoot, mdxPath));
      } catch (e) {
      }
    }
